...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow p-6">
        <form method="POST" action="{{ route('users.store') }}" class="space-y-5">
            ...
            <div>
                <label for="password" class="block text-sm font-medium text-gray-700 mb-1">Password * (Min. 6 karakter)</label>
            ...
            <div>
                ...
                <select name="role" id="role" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <option value="staff" {{ old('role') == 'staff' ? 'selected' : '' }}>Staff (Input Arsip)</option>
                    <option value="kades" {{ old('role') == 'kades' ? 'selected' : '' }}>Kepala Desa (View Laporan)</option>
                </select>
